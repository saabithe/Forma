    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8 bg-bg">
        <div className="card-strong rounded-2xl p-10 max-w-lg w-full text-center">
          ...
          <button onClick={() => onComplete(result)} className="w-full py-4 rounded-xl bg-primary text-white font-display font-semibold hover:bg-primary-light transition-colors text-lg flex items-center justify-center gap-2">
    ...
    <div className="min-h-screen flex items-center justify-center p-8 bg-bg">
      <div className="card-strong rounded-2xl p-10 max-w-lg w-full">
        ...
        <div className="flex gap-1 mb-8">
          {STEPS.map((_, i) => (
            <div key={i} className={`flex-1 h-1 rounded-full ${i <= step ? 'bg-primary' : 'bg-gray'}`} />
          ))}
        </div>
